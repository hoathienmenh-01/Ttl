...
async function seed() {
  ...
  await db.insert(dungeonTemplatesTable).values([
    { id: "thanh_khe_coc",    name: "Thanh Khê Cốc",      description: "Thung lũng suối xanh nơi dã thú nguy hiểm ẩn náu. Bí cảnh đầu tiên của đệ tử Hoa Thiên Môn.", element: "moc",  minRealm: "phamnhan", difficulty: "easy",   stages: 3, monsterName: "Thanh Lang Yêu",  monsterHp: 300,  monsterAtk: 40,  monsterDef: 8,  expReward: 300,   linhThachReward: 150,  dropItems: ["linh_chi", "hoi_khi_dan"], modifiers: { "poison": 0.08 }, bossModifiers: { "poison_damage": 0.05 } },
    { id: "co_quang_mo",      name: "Cổ Quặng Mỏ",        description: "Mỏ quặng cổ đại bị Thổ quái chiếm giữ. Tài nguyên quý hiếm chờ người dũng cảm.",             element: "tho",  minRealm: "phamnhan", difficulty: "easy",   stages: 3, monsterName: "Thổ Quái Nhỏ",   monsterHp: 450,  monsterAtk: 55,  monsterDef: 20, expReward: 400,   linhThachReward: 200,  dropItems: ["thiet_tinh", "tho_cot_thu"], modifiers: { "armor": 0.15 }, bossModifiers: { "armor": 0.2 } },
    { id: "hoa_nguyen_quan",  name: "Hỏa Nguyên Quan",    description: "Quan ải cháy bỏng nơi Hỏa linh thú làm chủ. Nguy hiểm bậc trung.",                            element: "hoa",  minRealm: "luyenkhi", difficulty: "medium", stages: 4, monsterName: "Hỏa Hồ Linh",    monsterHp: 800,  monsterAtk: 100, monsterDef: 30, expReward: 1000,  linhThachReward: 500,  dropItems: ["hoa_tinh_thach", "kim_quang_dan"], modifiers: { "fire_damage": 0.12 }, bossModifiers: { "fire_damage": 0.15 } },
    { id: "tuyet_son_dong",   name: "Tuyết Sơn Động",     description: "Hang tuyết trên đỉnh núi cao, Băng hệ linh thú canh gác kho báu cổ.",                         element: "bang", minRealm: "luyenkhi", difficulty: "medium", stages: 4, monsterName: "Băng Lang Yêu",   monsterHp: 1000, monsterAtk: 120, monsterDef: 40, expReward: 1200,  linhThachReward: 600,  dropItems: ["thuy_linh_thu", "ngoc_boi_linh"], modifiers: { "freeze": 0.1 }, bossModifiers: { "freeze": 0.12 } },
    { id: "loi_nguyen_dinh",  name: "Lôi Nguyên Đỉnh",   description: "Đỉnh núi nơi sấm sét không ngừng, nơi Lôi hệ cường giả ẩn tu.",                               element: "loi",  minRealm: "trucco",   difficulty: "hard",   stages: 5, monsterName: "Lôi Ưng Thần",   monsterHp: 3000, monsterAtk: 300, monsterDef: 80, expReward: 5000,  linhThachReward: 2500, dropItems: ["ngoc_than", "hoa_thien_ling"], modifiers: { "regen": 0.06 }, bossModifiers: { "regen": 0.08 } },
    { id: "am_linh_den",      name: "Âm Linh Điện",       description: "Điện thờ cổ của Tịch Thiên Điện bị bỏ hoang. Nơi đây chứa bí mật về đại kiếp thượng cổ.",     element: "am",   minRealm: "trucco",   difficulty: "hard",   stages: 5, monsterName: "Tịch Thiên Hộ Vệ", monsterHp: 4000, monsterAtk: 400, monsterDef: 100, expReward: 8000, linhThachReward: 4000, dropItems: ["hoa_thien_ling", "truc_co_dan"], modifiers: { "mp_drain": 0.08 }, bossModifiers: { "mp_drain": 0.1 } },
  ]).onConflictDoNothing();
  ...
  await db.insert(alchemyRecipesTable).values([
    ...
    { id: "recipe_kim_dan_tam_an", name: "Kết Kim Đan Tâm Ấn", description: "Ổn định đan tâm bằng Ngọc Thần Thạch, Hỏa Tinh Thạch và Trúc Cơ Đan. Công thức hỗ trợ củng cố cảnh giới, không thay thế tu luyện.", inputItems: [{ itemId: "ngoc_than", qty: 1 }, { itemId: "hoa_tinh_thach", qty: 2 }, { itemId: "truc_co_dan", qty: 1 }], outputItemId: "kim_dan_tam_an", outputQty: 1, successRate: 0.65, linhThachCost: 900, requiredRealm: "kimdan", sortOrder: 9 },
    { id: "recipe_am_linh_phu", name: "Khắc Âm Linh Trấn Phù", description: "Dùng Hoa Thiên Linh Ngọc và Mộc Linh Phù làm nền, khắc thêm Hỏa Tinh Thạch để trấn âm khí Âm Linh Điện.", inputItems: [{ itemId: "hoa_thien_ling", qty: 1 }, { itemId: "moc_linh_phu", qty: 1 }, { itemId: "hoa_tinh_thach", qty: 1 }], outputItemId: "am_linh_phu", outputQty: 1, successRate: 0.62, linhThachCost: 1100, requiredRealm: "kimdan", sortOrder: 10 },
    { id: "recipe_nguyen_anh_tam_hoa", name: "Dưỡng Nguyên Anh Tâm Hỏa", description: "Lấy Kim Đan Tâm Ấn làm dẫn, hòa cùng Hoa Thiên Linh Ngọc và Ngọc Thần Thạch để dưỡng Nguyên Anh mới sinh.", inputItems: [{ itemId: "kim_dan_tam_an", qty: 1 }, { itemId: "hoa_thien_ling", qty: 1 }, { itemId: "ngoc_than", qty: 2 }], outputItemId: "nguyen_anh_tam_hoa", outputQty: 1, successRate: 0.55, linhThachCost: 1800, requiredRealm: "nguyenanh", sortOrder: 11 },
  ]).onConflictDoNothing();
