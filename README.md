...
- [x] Multi-floor dungeon boss MVP: mỗi bí cảnh có floor thường, boss cuối, reward/drop tổng hợp server-side
...
### Luyện Đan
- [x] 11 alchemy recipes dùng hệ craft hiện có, server-authoritative
- [x] Recipe cao cấp Kim Đan/Nguyên Anh có realm gating, nguyên liệu rõ ràng và success rate cân bằng
- [x] UI hiển thị cảnh giới yêu cầu, tỷ lệ thành công, nguyên liệu thiếu và trạng thái thiếu Linh Thạch

### Linh Thú / Companion
- [x] 4 pet templates phẩm cấp Phàm/Linh, không bán bằng tiền
- [x] Người chơi có thể sở hữu và chọn 1 pet active
- [x] Pet unlock qua starter/mission/boss/event, không còn free claim toàn bộ catalog
- [x] Pet có level/exp MVP, nhận EXP nhỏ từ dungeon/boss/mission và có level cap
- [x] Pet hỗ trợ combat nhẹ server-side: ATK/DEF nhỏ và proc sát thương có hard cap
- [x] Trang Linh Thú hiển thị catalog, level, exp, nguồn mở khóa, pet đã sở hữu và active pet
...
artifacts/
  api-server/
    src/
      routes/         — 17 route files (auth, character, cultivation, boss, mission, npc, skill, dungeon, pet, ...)
      middlewares/    — auth.ts (token validation)
      lib/            — realms.ts (21 cảnh giới), elements (5 hệ)
      seed.ts         — Seed dữ liệu mẫu
  tu-tien-lo/
    src/
      pages/          — 18 trang (home, cultivation, dungeon, boss, mission, npc, skill, pet, inventory, ...)
      components/     — GameShell layout
      lib/            — hooks.ts, api.ts, constants.ts
lib/
  db/
    src/schema/       — schema modules cho users, characters, inventory, bosses, missions, chat, sects, market, topup, npcs, skills, pets, achievements, npc_affinity, ...
...
GET  /api/pet/catalog
GET  /api/pet/mine
POST /api/pet/:id/claim
POST /api/pet/:id/equip
POST /api/pet/:id/unequip
GET  /api/achievement
...
- **Pet combat server-side**: pet active chỉ tăng chỉ số/proc trong cap nhỏ, server guard không equip pet chưa sở hữu
...
- [ ] Apply DB rollout/preflight trên môi trường thật cho `active_slot`, `affinity_required`, `character_npc_affinity`, `pet_templates`, `character_pets`
...
- [ ] More alchemy recipe unlock sources qua quest/NPC affinity/event
- [ ] Pet skill flavor/proc log nâng cao và unlock qua quest rewards đầy đủ hơn
- [ ] PvP arena
- [ ] Guild war
